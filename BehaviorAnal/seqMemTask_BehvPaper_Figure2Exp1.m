...
%% parameters
StudyName_list = {'Exp1', 'Exp2'};
StudyID   = 1;
StudyName = StudyName_list{StudyID};
...
subjList_young = {'3NL2IF', '03-08-2023_17h40.53.123'; ...
...
subjList_old = {'WBG259', '03-09-2023_08h23.30.024'; ...
    ...
    for iSub = 1 : subLen
        ...
        for ij = 1 : 2 % 2 types of report trials: 1) item-pos; 2) pos-item;
            if ij == 1     % retrieval order: object-location
                slotCorr_con = slotCorr_marg_con(testOrd == 0, :);
                slotCorr_pos = slotCorr_marg_pos(testOrd == 0, :);
            elseif ij == 2 % retriveal order: location-object
                slotCorr_con = slotCorr_marg_con(testOrd == 1, :);
                slotCorr_pos = slotCorr_marg_pos(testOrd == 1, :);
            end
            binds_conPctr_counts = cell(2, 4); % 2: (Item|Pos) and (Pos|Item)
            for ijT = 1 : size(slotCorr_pos, 1)
                slotCorr_con_ij = slotCorr_con(ijT, :);
...
FBdata_folder = [bhvDataDir, '/AgingStudy-FlexibleBinding/SummaryData/']; % flexible binding data folder
if ~exist(FBdata_folder, 'dir')
    mkdir(FBdata_folder);
end
...
for iGrp = 1 : nGroup % YA and OA
    ...
    FA_lure_iGrp = FA_lure_group{iGrp};
...
save([FBdata_folder, 'Fig2D_FA_lure_overall_YAOA_stats_Exp1.mat'], 'FA_lure_overall_YAOA');
...
for iGrp = 1 : nGroup % YA and OA
    ...
    transAcc_drop_mjAvg_iGrp = transAcc_drop_mjAvg_group{1, iGrp};
    [transAcc_avg, transAcc_sem] = Mean_and_Se(transAcc_drop_mjAvg_iGrp)
end

%% save data for Figure 2E in the FlexibleBinding paper
save([FBdata_folder, 'Fig2E_transAcc_plot_group_Exp1.mat'], 'transAcc_plot_group');
save([FBdata_folder, 'Fig2E_transAcc_plot_mjAvg_group_Exp1.mat'], 'transAcc_plot_mjAvg_group');
...
for iGrp = 1 : nGroup % YA and OA
    ...
    for ij = 1 : 2 % marginal and reconstruction
        transAcc_ij = transAcc_plot_group{ij, iGrp};
...
save([FBdata_folder, 'Fig2E_transAcc_overall_group_stats_Exp1.mat'], 'transAcc_overall_group');
...
for iGrp = 1 : nGroup % YA and OA
    ...
    for ij = 1 : 2 % marginal and reconstruction
        binds_conPctr_group_ij = binds_conPctr_group{ij, iGrp}(:, :, 1 : 2);
...
%% ------save the above data for subsequent statistical tests------
save([FBdata_folder, 'Fig2G_binds_overall_group_stats_Exp1.mat'], 'binds_overall_group');
save([FBdata_folder, 'Fig2HI_binds_overall_diff_group_stats_Exp1.mat'], 'binds_overall_diff_group');
