function [minuslli, Mc, Mp, Mfc_c, Mfc_p, Mb] = SMT_featureIndependentVM(params, nImg, nPos, nTrans, trialLen, stim_encode, disp_con, disp_pos, disp_rec, resp_con, resp_pos, resp_rec, context_arr, testOrd_arr, reconsOnly, Minit, angList, angSeq_encode, bindingDirec)
% write by XR @ April 4 2024
% For the independent encoding, introducing uncertainty for the positions
...
kappa   = params(4); % precision in the VM function
eta0_c  = params(5); % scaling factor of the item-to-context association, content
eta0_p  = params(6); % position
eta     = params(7); % exponential parameter of the item-to-context association
beta    = params(8); % inverse temperature
%%
states = nImg;
...
Mfc_p = zeros(nPos, 4);
% one-hot matrix for the position transitions
I_vm = zeros(nPos, nPos);
for ip = 1 : nPos
    mu = angList(ip); % assuming all 8 positions are known
...
for iT = 1 : trialLen % only those non-reconstruction-only trials, 48 trials
    ...
    for ii = 1 : nTrans
        ...
        if ii < nTrans % only 4 transitions: e.g., A-B, B-C, C-D, D-E
            ...
            Mp(s_i_pos, :) = (1 - alpha_p) .* Mp(s_i_pos, :) + alpha_p .* (I_vm(s_Tgt_pos, :) + gamma .* Mp(s_Tgt_pos, :));
        end
