function [minuslli, Mc, Mp, Mfc_c, Mfc_p, Mb] = SMT_featureCompetition(params, nImg, nPos, nTrans, trialLen, stim_encode, disp_con, disp_pos, disp_rec, resp_con, resp_pos, resp_rec, context_arr, testOrd_arr, reconsOnly, Minit, angList, angSeq_encode, bindingDirec)
% =================================================
% write by XR @ 03/28/2024
% =================================================
...
beta    = params(11); % inverse temperature
% ---------- Modifications  ----------
% added by XR @ Oct 28 2025
% For the displayed sequence per trial, the decay parameter weighed the
% older binding associations and one learnt again via item-location
% association
% ------------------------------------------------------------
...
for iT = 1 : trialLen % only those non-reconstruction-only trials, 48 trials
    ...
    for ii = 1 : nTrans
        ...
        % !!!!!! added by rxj @ August 23, 2024 !!!!!!
        % If kappa > 700 and cos(angList - mu)=1, the output from exp()
        % will be Inf. Therefore the minuslli=NaN and this will bring
        % errors for bads.
        varyPart = exp(kappa .* cos(angList - mu));
        varyPart(isinf(varyPart)) = exp(700);
        % !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        %delta_Mb = samePart * exp(kappa .* cos(angList - mu)); % angList: the eight angle used for each participant
        delta_Mb = samePart * varyPart; 
